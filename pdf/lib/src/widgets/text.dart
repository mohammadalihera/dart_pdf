    ...
    void _buildLines() {
      for (final span in _preprocessed!) {
        ...
        if (span is TextSpan) {
          ...
          final spanLines = (_textDirection == TextDirection.rtl ? arabic.convert(span.text!) : span.text)!.split('\n');
...
class Text extends RichText {
  Text(String text,
      ...
      bool isAnsiSupportedText = false,
      double textScaleFactor = 1.0,
      int? maxLines,
      TextOverflow? overflow,
      TextType textType = TextType.Default})
      : super(
          text: isAnsiSupportedText && Font.isAnsiSupportedText
              ? const TextSpan().toAnsiOrUnicodeText(
                  text: text,
                  textType: textType,
                  ansiFont: Font.ansi_font,
                  unicodeFont: Font.unicode_font,
                  textStyle: style)
              : TextSpan(
                  text: text,
                  style: Font.unicode_font == null
                      ? style
                      : style != null
                          ? style.copyWith(font: Font.unicode_font)
                          : TextStyle(font: Font.unicode_font)),
          textAlign: textAlign,
          softWrap: softWrap,
