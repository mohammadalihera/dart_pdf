...
abstract class TableColumnWidth {
  ...
  _ColumnLayout layout(Widget child, Context context, BoxConstraints constraints);
}
...
class Table extends Widget with SpanningWidget {
  ...
  factory Table.fromTextArray({
    ...
    double cellHeight = 0,
    TextType headerTextType = TextType.Default,
    ...
    List<int> ansiSupportedCellIndexes = const [],
    List<int> multiLineTextCellIndexes = const [],
    ...
    if (headers != null) {
      ...
      for (final dynamic cell in headers) {
        final cellIndex = headers.indexOf(cell);
        ...
        final cellContent = Container(
          ...
          child: Text(headerFormat == null ? cell.toString() : headerFormat(tableRow.length, cell),
              style: headerStyle,
              isAnsiSupportedText: Font.isAnsiSupportedText,
              textType: headerTextType,
              maxLines: multiLineTextCellIndexes.contains(cellIndex) ? null : 1),
        );

        tableRow.add(
          multiLineTextCellIndexes.contains(cellIndex)
              ? cellContent
              : SizedBox(
                  height: headerHeight > 0 ? headerHeight : 20,
                  child: FittedBox(
                      alignment: headerAlignments[tableRow.length] ?? headerAlignment,
    ...
    for (final row in data) {
      ...
      if (rowNum < headerCount) {
        ...
        for (final dynamic cell in row) {
          final cellIndex = row.indexOf(cell);
          ...
          final cellContent = Container(
            ...
            child: Text(cellFormat == null ? cell.toString() : cellFormat(tableRow.length, cell),
                style: isOdd ? oddCellStyle : cellStyle,
                isAnsiSupportedText: ansiSupportedCellIndexes.contains(cellIndex),
                textAlign: textAlign,
                maxLines: multiLineTextCellIndexes.contains(cellIndex) ? null : 1),
          );
          ...
          tableRow.add(
            multiLineTextCellIndexes.contains(cellIndex)
                ? cellContent
                : SizedBox(
                    height: cellHeight > 0 ? cellHeight : 20.0,
                    child: FittedBox(alignment: align, fit: BoxFit.scaleDown, child: cellContent),
                  ),
          );
        }
      }
  ...
  void layout(Context context, BoxConstraints constraints, {bool parentUsesSize = false}) {
    ...
    for (final row in children) {
      ...
      for (final child in row.children) {
        final columnWidth = columnWidths != null && columnWidths![n] != null ? columnWidths![n]! : defaultColumnWidth;
