...
List<TableRow> buildTable({required Context? context, int count = 10, bool repeatHeader = false}) {
  final rows = <TableRow>[];
  {
  ...
  for (var y = 0; y < count; y++) {
    ...
    final tableRow = <Widget>[
      Container(margin: const EdgeInsets.all(5), child: Text('${h.toInt()}°', style: Theme.of(context!).tableCell)),
      Container(
          margin: const EdgeInsets.all(5),
      ...
      Container(margin: const EdgeInsets.all(5), child: Text(color.toHex(), style: Theme.of(context).tableCell)),
    ];
...
void main() {
  ...
  test('Table Widget multi-pages with header', () {
    pdf.addPage(MultiPage(
        build: (Context context) => <Widget>[
              Table(
                children: buildTable(context: context, count: 200, repeatHeader: true),
                border: TableBorder.all(),
                tableWidth: TableWidth.max,
              ),
            ]));
  ...
  test('Table Widget multi-pages short header', () {
    pdf.addPage(MultiPage(
        build: (Context context) => <Widget>[
              SizedBox(height: 710),
              Table(
                children: buildTable(context: context, count: 4, repeatHeader: true),
                border: TableBorder.all(),
                tableWidth: TableWidth.max,
              ),
            ]));
          ...
          return <Widget>[
            Table(
              ...
              children: List<TableRow>.generate(
                ...
                (int index) {
                  final align = TableCellVerticalAlignment.values[index % TableCellVerticalAlignment.values.length];
