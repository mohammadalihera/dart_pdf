...
class WidgetWrapper extends pw.ImageProvider {
  ...
  static Future<WidgetWrapper> fromKey({
    ...
    final wrappedWidget = key.currentContext!.findRenderObject() as RenderRepaintBoundary;
  ...
  static Future<WidgetWrapper> fromWidget({
    ...
    if (!constraints.hasBoundedHeight || !constraints.hasBoundedHeight) {
      throw Exception('Unable to convert an unbounded widget. Add maxWidth and maxHeight to the constraints.');
    }
    ...
    final computedConstraints = prop.properties.whereType<DiagnosticsProperty<BoxConstraints>>().first.value;

    if (computedConstraints == null || !computedConstraints.hasBoundedWidth || !computedConstraints.hasBoundedWidth) {
    ...
    final renderView = RenderView(
      ...
      configuration: ViewConfiguration(size: view.physicalSize, devicePixelRatio: view.devicePixelRatio),
      view: view,
    );
